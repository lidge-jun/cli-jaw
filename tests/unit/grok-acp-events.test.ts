...
test('null optional metadata stays absent and the largest exact counter remains valid', () => {
    for (const result of [{ _meta: null }, { _meta: { usage: null } }]) assert.equal(grokUsage(result), null);
    assert.deepEqual(grokUsage({ _meta: { usage: { inputTokens: Number.MAX_SAFE_INTEGER } } }),
        { kind: 'usage', inputTokens: Number.MAX_SAFE_INTEGER });
});
