...
test('model metadata rejects hidden keys, executable hooks and array/node overflows before dispatch', async t => {
    const f = fixture(t); await f.start(); const count = f.writes.length;
    let invoked = false;
    ...
    const invalid = [{ [Symbol('key')]: 1 }, hidden, { value: new Array(2) }, { value: extended },
        hook, { value: Array.from({ length: 4095 }, () => 0) }];
    for (const meta of invalid) {
        await assert.rejects(f.session.setModel('m2', meta), /acp_invalid_model_meta/);
        assert.equal(f.writes.length, count); assert.equal(f.session.idle, true);
    }
    assert.equal(invoked, false);
    // Small numeric leaves stay below the byte cap, isolating the node-count boundary.
    const valid = { value: Array.from({ length: 4094 }, () => 0) };
    await f.session.setModel('m2', valid);
    assert.deepEqual(f.writes.at(-1)!.params, { sessionId: 'native', modelId: 'm2', _meta: valid });
});
