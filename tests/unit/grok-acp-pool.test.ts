...
function fixture(t: TestContext) {
    ...
    const options: GrokAcquireOptions = {
        ...
        binary: 'fake-acp', env: { XAI_API_KEY: 'fixture-key-a', GROK_AUTH: 'fixture-inline-auth-a', HOME: '/fixture/home-a',
...
for (const field of ['XAI_API_KEY', 'GROK_AUTH', 'HOME', 'USERPROFILE', 'GROK_HOME', 'GROK_AUTH_PATH']) {
    ...
    test(`${field} rotation replaces an alive idle session without relying on exit`, async t => {
        const f = fixture(t), first = await acquireGrokRuntime(f.options), old = f.sessions[0]!;
        first.release(); assert.equal(old.alive, true); assert.equal(old.closeCalls, 0);
        const second = await acquireGrokRuntime({ ...f.options, env: { ...f.options.env, [field]: 'changed-fixture-value' } });
        try {
            assert.notEqual(second.session, first.session);
            assert.equal(second.reused, false); assert.equal(old.closeCalls, 1);
            assert.equal(f.creations.length, 2); assert.equal(f.creations[1]!.env[field], 'changed-fixture-value');
