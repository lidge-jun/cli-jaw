...
mock.module('../../src/core/bus.js', { namedExports: {
    broadcast: (name: string, value: Record<string, unknown>) => { broadcasts.push({ name, value }); },
} });
...
mock.module('../../src/agent/runtime/projection.js', { namedExports: { RuntimeProjection: class { start() {} close() {} } } });
...
function fixture(worker = false) {
    ...
    let send: NativeRuntimeSession['send'] = async prompt => { makeClaudeUserMessage(prompt); return outcome; };
