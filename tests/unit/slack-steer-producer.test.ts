...
for (const prompt of ['fixture prompt', '/continue', '/reset']) {
    test(`actual idle Slack restart preserves delivery ownership: ${prompt}`, async () => {
        ...
        // _fromSteer is the #655 marker pipeline.ts turns into fromSteer on the terminal broadcast.
        assert.deepEqual(calls[0]?.meta, { ...expected, chatSessionId: sessionId, sessionId, _fromSteer: true, _skipInsert: true });
    });
}
...
test('restart rejection retains captured Slack context after caller mutation', async () => {
    ...
    assert.deepEqual(terminal, { text: '[error] fixture failure', error: true, origin: 'slack', scope, sessionId,
        requestId, target, chatId: target.targetId, remoteKey, replyViaTarget: true, fromSteer: true, mode: 'restart' });
});
...
for (const source of ['web', 'telegram', 'slack']) {
    test(`restart leaves legacy metadata unchanged without a valid Slack target: ${source}`, async () => {
        ...
        // Without a Slack target the #654 capture stays out; what remains is the #655
        // kill-steer identity current dev already ships.
        const legacyTarget = { ...target, channel: 'telegram' };
        assert.deepEqual(events, [{ prompt: 'fixture', origin: source, scope, sessionId, target: legacyTarget, chatId: 'old-chat', requestId, remoteKey, replyViaTarget: true, mode: 'kill-steer' }]);
        assert.deepEqual(calls[0]?.meta, { origin: source, scope, chatSessionId: sessionId, requestId, target: legacyTarget, chatId: 'old-chat', remoteKey, replyViaTarget: true, _fromSteer: true, _skipInsert: true });
    });
}
