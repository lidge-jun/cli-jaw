...
export class ClaudeSdkChildren {
    ...
    stopOwner(context: RuntimeEventContext): void {
        for (const child of this.children.values()) {
            // Unknown prelink frames cannot safely cross a Stop into another send.
            if (!child.owner || sameOwner(child.owner.context, context)) this.finish(child, 'stopped');
        }
    ...
    resolveTool(toolUseId: string): ClaudePermissionOwner | null {
        ...
        if (!this.declaredTools.has(toolUseId) || !child?.context || !this.active(child)) return null;
    ...
    private record(child: Child, context: RuntimeEventContext, body: RuntimeEventBody): RuntimeEvent | null {
        // Stop snapshots remain recordable after active=false. Generation is the fence here.
        if (!child.owner?.isCurrent()) return null;
    ...
    private deliver(child: Child, frame: Obj): void {
        ...
        for (const toolId of this.frameTools(frame, true)) this.declaredTools.add(toolId);
    }
    ...
    private finish(child: Child, status: Status): void {
        ...
        if (!child.owner?.isCurrent()) return;
