...
export const MAX_SAVED_ACTIVITY_ANSWER_BYTES = 16 * 1024 * 1024;
...
export interface SavedActivityAnswer {
    id: number;
    role: 'assistant';
    content: string;
    trace_run_id: string;
 ...
 */
async function readPage(options: ActivityReadOptions, path: string, maxBytes = PAGE_BYTES): Promise<unknown> {
    options.signal.throwIfAborted();
    const raw = await options.read(path, options.signal);
    options.signal.throwIfAborted();
    const serialized = JSON.stringify(raw);
    if (serialized !== undefined && encoder.encode(serialized).length > maxBytes) throw new Error('activity_page_limit');
...
export async function readActivityRun(options: ActivityReadOptions & { runId: string; after?: number }): Promise<ActivityRunReadResult> {
    ...
    for (let pageIndex = 0; pageIndex < MAX_PAGES; pageIndex++) {
        ...
        for (const value of page['events']) {
            ...
            if (turnId !== undefined && event.turnId !== turnId) throw new Error('activity_turn_changed');
            turnId = event.turnId;
...
/** Exact saved MESSAGE, never a canonical/diagnostic preview or latest-answer guess. */
export async function readSavedActivityAnswer(options: ActivityReadOptions & { runId: string }): Promise<SavedActivityAnswer | null> {
    if (!id(options.runId) || !id(options.sessionId)) throw new Error('invalid_activity_identity');
    const query = new URLSearchParams({ session: options.sessionId });
    const page = await readPage(options, `/api/messages/by-trace/${encodeURIComponent(options.runId)}?${query}`,
        MAX_SAVED_ACTIVITY_ANSWER_BYTES);
    if (!record(page)) throw new Error('invalid_saved_activity_answer');
    const message = page['message'];
    if (message === null) return null;
    if (!record(message) || !cursor(message['id']) || message['id'] === 0 || message['role'] !== 'assistant'
        || typeof message['content'] !== 'string' || message['trace_run_id'] !== options.runId
        || message['session_id'] !== options.sessionId) throw new Error('invalid_saved_activity_answer');
    return { id: message['id'], role: 'assistant', content: message['content'],
        trace_run_id: options.runId, session_id: options.sessionId };
}
