...
export async function verifySlackTables(
...
): Promise<{ ok: boolean; verifiedTables: number; tableContent: TableContentStatus; verifiedFeatures?: string[]; reason?: string }> {
    ...
    }, { ...options, form: true, timeoutMs: 10000, maxResponseBytes: 1048576 });
    const unchecked: TableContentStatus = content?.length ? 'failed' : 'not_checked';
    if (!result.ok) return { ok: false, verifiedTables: 0, tableContent: unchecked, reason: result.error ?? 'readback_failed' };
    try {
        ...
        if (!message) return { ok: false, verifiedTables: 0, tableContent: unchecked, reason: 'posted_message_not_found' };
        ...
        if (!ok) return { ok: false, verifiedTables: 0, tableContent: content?.length ? 'failed' : 'not_checked', reason: 'table_count_or_shape_mismatch' };
        ...
        if (!compared.ok) return { ...compared, tableContent };
        ...
        return missing.length ? { ok: false, verifiedTables: compared.verifiedTables, tableContent, reason: `missing_rich_features:${missing.join(',')}` }
            : { ok: true, verifiedTables: compared.verifiedTables, tableContent, verifiedFeatures: features };
    } catch (error) {
        if (!(error instanceof RangeError)) throw error;
        return { ok: false, verifiedTables: 0, tableContent: content?.length ? 'failed' : 'not_checked', reason: error.message };
    }
}
