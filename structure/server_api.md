...
- `lastStartFailure` is an optional sibling on those same rows: `{ code, at }` for the last native runtime that failed before acquiring a lease. `code` is one provider-independent identifier admitted by the runtime layer's safe-code shape, never provider text; `at` is epoch milliseconds. It is a live per-turn record, so it is neither a cached probe field nor a member of `runtimeSelection`, and it is omitted rather than null when nothing is recorded. A later start that reaches its lease clears it; `?force=1` does not.
