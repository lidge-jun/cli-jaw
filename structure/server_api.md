...
모든 Slack heartbeat runner는 같은 target에 `enforceDestination` grant를 예약한다. print child는 grant header를 제시하므로 Auto/full-local에서도 target 생략은 저장 목적지로 고정되고 다른 target은 `slack_destination_mismatch`다. native/pool·employee·script는 새 per-turn 환경 header를 받을 수 없으므로 reservation 동안 headerless Slack send가 `slack_enforced_destination_grant_required`로 거절되고 서버가 최종 전송을 소유한다. 일반 interactive Auto grant는 이 flag가 없으며 scheduled reservation 밖에서는 instance-wide 권한 계약을 유지한다.
