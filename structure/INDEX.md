...
| Claude E runtime wrapper | `native/jaw-claude-i/`, `src/agent/claude-i-runtime.ts`, `src/agent/args.ts`, `src/agent/events.ts`, `src/core/config.ts`, `src/cli/readiness.ts`, `bin/commands/doctor.ts` | `claude-i` is an experimental registry provider backed by the `claude-e` runtime surface; helper discovery honors `CLAUDE_E_BIN`, bundled npm `claude-e`, PATH `claude-e`, compatibility `claude-exec`, then legacy `jaw-claude-i` / `claude-i` fallbacks. Runtime events remain `agent:claude-i:*`, and `npm run build:claude-exec` / `npm run test:claude-exec` remain native fallback build/test aliases. |
