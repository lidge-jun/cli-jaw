...
│   │   │   │   ├── config.ts ← bounded negotiated model/effort selection (255L)
...
│   │   ├── settings.ts       ← settings/prompt/project pick/git summary/heartbeat-md/MCP/registry/status/quota/copilot + Pi profile register/model discovery 라우트 + CLI_KEYS 기반 quota parity/status-only metadata (767L)
