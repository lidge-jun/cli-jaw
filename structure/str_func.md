...
│   │   ├── actions.ts        ← snapshot/click/type/navigate/screenshot + browser primitive actions (621L)
│   │   ├── primitives.ts     ← low-level CDP primitives (294L)
│   │   ├── vision.ts         ← vision-click 파이프라인 + Codex provider(--ephemeral, stdin ignore) + guardrail options (367L)
...
│   │   ├── occlusion.ts      ← 클릭 지점 hit-test 소스 + 가림 판정(자손/label/presentational은 통과, 미확인은 fail-open) (127L)
