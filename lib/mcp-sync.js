 ...
 *   CODEX_ACTIVE  → copy to ~/.cli-claw/skills/      (CLI auto-trigger)
 *   CODEX_REF     → copy to ~/.cli-claw/skills_ref/   (AI reads on-demand)
 *   CODEX_DELETE  → skip entirely (duplicated by OpenClaw skills)
 *
 * OpenClaw skills (bundled skills_ref/):
 *   OPENCLAW_ACTIVE → copy to ~/.cli-claw/skills/     (auto-activated)
 *   Everything else stays in skills_ref/               (reference only)
 */
export function copyDefaultSkills() {
    ...
    const CODEX_DELETE = new Set([
        // Duplicated by OpenClaw skills already in skills_ref
        // imagegen covers openai-image-gen, transcribe covers whisper-api
    ]);

    // Everything else from Codex → ref
    ...
    if (fs.existsSync(codexSkills)) {
        ...
        let activeCount = 0, refCount = 0, skipCount = 0;

        for (const skill of skills) {
            const src = join(codexSkills, skill.name);

            if (CODEX_DELETE.has(skill.name)) {
                skipCount++;
                continue;
            }
        ...
        console.log(`[skills] Codex: ${activeCount} active, ${refCount} ref, ${skipCount} skipped`);
    }

    // ─── OpenClaw Classification ─────────────────────
    // Bundled skills_ref/ from npm package → ~/.cli-claw/skills_ref/
    ...
    if (fs.existsSync(packageRefDir)) {
        ...
        if (refCopied > 0) console.log(`[skills] OpenClaw: ${refCopied} skills → ref`);
    }

    // OpenClaw skills to auto-activate
    const OPENCLAW_ACTIVE = ['browser', 'notion', 'memory'];
    for (const id of OPENCLAW_ACTIVE) {
