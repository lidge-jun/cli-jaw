...
export function settleActivityFallbackOutput(ctx: TuiContext, key: string, finalText: string | null | undefined): void {
    const printed = settleActivityFallbackPreviews(ctx.store.transcript, key);
    if (!printed || ctx.displayMode !== 'line') return;
    process.stdout.write(finalText === undefined ? '\n[Provisional output ended; checking the saved answer.]\n'
